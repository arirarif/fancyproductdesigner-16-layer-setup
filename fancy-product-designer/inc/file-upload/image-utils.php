...
if (!class_exists('FPD_Image_Utils')) {
	...
	class FPD_Image_Utils
	{
		...
		public static function apply_imagick_filter($image_path = null, $filter = 'grayscale')
		{

			$result = array();

			$origin_ext = strtolower(pathinfo($image_path)['extension']);

			if (!$image_path) return;

			if (class_exists('Imagick') && in_array($filter, array('grayscale', 'black_white', 'threshold', 'threshold_negative'))) {
				...
				if ($origin_ext == 'png' && in_array($filter, array('grayscale', 'black_white')))
					$image->setImageAlphaChannel(Imagick::ALPHACHANNEL_ACTIVATE);

				if (in_array($filter, array('black_white', 'threshold', 'threshold_negative'))) {

					// Apply a threshold to get a pure black and white image
					$image->thresholdImage(Imagick::getQuantum() * 0.5); // 0.5 is 50%, adjust as needed

				}

				//replace jpg,svg extension with png
				$image_path = preg_replace('/\.(jpg|jpeg|svg)$/i', '.png', $image_path);

				if (in_array($filter, array('threshold', 'threshold_negative'))) {

					//save first otherwise png is empty
					...
					$image->transparentPaintImage('#ffffff', 0, 0, false);

					if ($filter == 'threshold_negative') {

						//whiten black pixels
						$image->setImageAlphaChannel(Imagick::ALPHACHANNEL_EXTRACT);
						$image->setImageBackgroundColor('#ffffff');
						$image->setImageAlphaChannel(Imagick::ALPHACHANNEL_SHAPE);
					}
				}
				...
				$result['error'] = 'You need to enable Imagick on your server in order to perfom any image transformation.';
			}
		...
		public static function is_image($url)
		{
			...
			if (self::is_base64($url)) {
				...
				// Create context with SSL verification disabled for self-signed certificates
				...
				$img_info = @getimagesize($url, $context);
