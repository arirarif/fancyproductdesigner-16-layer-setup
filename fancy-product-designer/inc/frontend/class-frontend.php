...
?>
			...
			?>
				...
				$configs = array(
					...
					'app_options' 	=> [
						...
						'dynamicDesigns' => is_array(get_option('fpd_dynamic_designs_modules', '{}')) ? get_option('fpd_dynamic_designs_modules', '{}') : json_decode(get_option('fpd_dynamic_designs_modules', '{}')),
						...
						'fileServerURL' => admin_url('admin-ajax.php') . '?action=fpd_custom_uplod_file&filter=' . $product_settings->get_option('imagick_filter') . '&nonce=' . wp_create_nonce('fpd_upload_file'),
						'elementParameters' => [
		...
		public static function add_customize_button()
		{
			...
			if (!$printful_enabled && (!is_array($fancy_content_ids) || sizeof($fancy_content_ids) === 0)) {
				return;
			}
...
<?php
		...
		public function ajax_custom_uplod_file()
		{
			...
			if (!isset($_GET['nonce']) || !wp_verify_nonce($_GET['nonce'], 'fpd_upload_file')) {
				wp_send_json_error(array('error' => 'Security check failed. Invalid nonce.'));
				die;
			}
